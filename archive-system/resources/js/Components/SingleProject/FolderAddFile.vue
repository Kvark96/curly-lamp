<template>
    <form @submit.prevent="form.post(route('files.store'), { preserveScroll: true })">
        <div class="grid grid-cols-3 grid-rows-3 w-full px-5 border border-slate-400 rounded bg-slate-300">
            <div class="py-4">
                <div v-if="form.wasSuccessful">File was uploaded!</div>
            ...
            <div class="py-2 text-right text-sm">
                <button
                    ...
                    type="submit" id="submit" :disabled="form.processing">
                    Save
                </button>
                <Link as="button"
